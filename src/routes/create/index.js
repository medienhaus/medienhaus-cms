...
import AlternativeAuthors from './AlternativeAuthors'
...
const Create = () => {
  ...
  const fetchSpace = useCallback(async () => {
    if (matrixClient.isInitialSyncComplete()) {
      // we make sure the old description gets deleted when creating a new project
      setDescription()
  ...
  return (
    <>
      ...
      {projectSpace && (
        <>{config.function.context && medienhausMeta.type === 'content' &&
          ...
          <section className="visibility">
            ...
            <p>
              <Trans t={t} i18nKey="visibilityInstructions1">
                Would you like to save your project as a draft or release it for publishing on the {config.platformName} platform? The released projects will be published for the Rundgang on October 29, 2021.
              </Trans>
            </p>
            ...
            {spaceObject
              ? (<>
                ...
                {config.function.context && !medienhausMeta.context &&
                  <>
                    <p>
          ...
          <section className="save">
            <button onClick={() => history.push(medienhausMeta.type === 'content' ? '/content' : '/pages')}>← {t('BACK TO OVERVIEW')}</button>
