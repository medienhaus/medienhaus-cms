...
const UdKLocationContext = ({ spaceRoomId }) => {
  ...
  const onRemoveFromLocation = async () => {
    ...
    await Matrix.removeSpaceChild(currentLocationContext.id, spaceRoomId)
      .catch(async () => {
        // If we can't add the space to a context we try to join the context first ...
        const joinRoom = await Matrix.getMatrixClient().joinRoom(currentLocationContext.id)
          .catch((error) => {
            console.log(error)
            setErrorMessage(error.data.error)
            return false
        ...
        if (joinRoom) {
          console.log('joined room {id} and retrying to add space to context', { id: currentLocationContext.id })
          // ... and then try to add the space to the context again
          await Matrix.removeSpaceChild(currentLocationContext.id, spaceRoomId)
            .catch((error) => {
              console.log(error)
